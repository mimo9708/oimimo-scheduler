...
| 收款时间线（Spec 26） | 详情页分期单专用：paid_at 升序列表（日期/金额/备注）+ 三数汇总（应收净额/已到账/待收）+ 只进不退警告条（payments-warn）；payments-* 全 token 化禁硬编码色，窄屏降级单列 |
| 收款编辑器（Spec 26） | 时间线内行内编辑（记一笔/改/删），document 级事件委托可 innerHTML 整卡替换；金额两位小数；提交走 dirtyFetch → toast + 整内容区刷新 |
| 小票分期标记（Spec 26） | .r-tag「分期」标签 + .r-sub 三数副行（应收/已到/待收，两位小数）；待收小票分期单按单列三数、到账/月度小票分期单按笔一行 |
